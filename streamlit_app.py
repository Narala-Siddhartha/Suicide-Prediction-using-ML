...
def home(data):
    ...
    st.image("./suicide.jpg",width=450)
     # app would work faster if you would not read and show the data set
...
def Feedback():
    st.header("Feedback")
    st.caption("We all need people who will give us feedback. That’s how we improve.")
    st.image("./feedback.jpg")
    st.write('Please Give your Feedback here:')
    #feedback = st.button('Feedback')
    #url="https://forms.gle/YV1JbrMyEc2Mbv8f8"
    st.write(f'''
    <a target="_blank" href="https://forms.gle/YV1JbrMyEc2Mbv8f8">
        <button>
            Feedback
        </button>
    </a>
    ...
    st.write('Want to see others reviews?, Check here:')

    st.write(f'''
    <a target="_blank" href="https://docs.google.com/spreadsheets/d/1i6P4UczJGY4UnGS3oxUjEvNwv0NJbB7J2tqkwXzJm_s/edit?usp=sharing">
        <button>
            Responses
        </button>
    </a>
...
if __name__ == "__main__":
    ...
    nav = st.sidebar.radio("Navigation",["Home","Feedback"])
    if nav == "Home":
        home(data)
        predict(data)
    if nav=="Feedback":
